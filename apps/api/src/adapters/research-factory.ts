...
/**
 * Whether the deployment serves a real map background.
 *
 * The map tiles are fetched by the Worker and served from our own origin, so
 * the browser never contacts a tile host and the strict CSP is untouched — the
 * same shape as the bottle-photo proxy. It needs the contact user agent OSM's
 * tile policy asks for, and defaults off, so the public build makes no external
 * tile request and the map falls back to its schematic form.
 */
export function mapTilesEnabled(environment: WorkerBindings): boolean {
  return (

...
/**
 * A wine's tastings, as a history.
 *
 * Every submitted tasting of this wine that the reader is allowed to see — which
 * inside a shared Space means everyone's, since scores and structured tasting
 * are shared there. What is NOT shared travels nowhere: a co-member's written
 * notes stay with them, and so does their venue, because a place is location
 * data about a person and a wine's history page is not where it belongs.
 *
 * Only the reader's own rows are openable in full; `getDeepNote` already refuses
 * anybody else's, and this list marks which is which so the interface need not
 * guess.
 */
export async function listWineTastings(
  database: D1Database,
  options: { principal: FirebasePrincipal; spaceId: string; wineId: string },
): Promise<TastingHistoryEntry[] | null> {
  // The reader must actively belong to the Space before any of this is readable.
  const member = await database
    .prepare(
      `SELECT reader.id FROM users reader
      JOIN space_memberships membership ON membership.user_id = reader.id
        AND membership.space_id = ? AND membership.status = 'active'
      WHERE reader.firebase_uid = ? AND reader.deleted_at IS NULL`,
    )
    .bind(options.spaceId, options.principal.firebaseUid)
    .first<{ id: string }>();
  if (member === null) return null;

  const rows = await database
    .prepare(
      `SELECT note.id, note.mode, note.score_100, note.tasted_at,
        author.display_name AS author_name,
        CASE WHEN author.firebase_uid = ? THEN 1 ELSE 0 END AS is_self,
        ctx.venue_name, ctx.venue_latitude, ctx.venue_longitude
      ...
      WHERE note.space_id = ? AND note.wine_id = ?
        AND note.state = 'submitted' AND note.deleted_at IS NULL
      ORDER BY note.tasted_at DESC
      LIMIT 50`,
    )
    .bind(options.principal.firebaseUid, options.spaceId, options.wineId)
    .all<{
  ...
  return rows.results.map((row) => {
    ...
    return {
      ...
      score100: row.score_100,
      tastedAt: row.tasted_at,
      // A place is the author's own. A point is only meaningful as a pair.
      venueLatitude: self && row.venue_longitude !== null ? row.venue_latitude : null,
      venueLongitude: self && row.venue_latitude !== null ? row.venue_longitude : null,
      venueName: self ? row.venue_name : null,
    };
  });
}

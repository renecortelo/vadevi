...
const wineTastingsRoute = createRoute({
  method: "get",
  path: "/api/v1/spaces/{spaceId}/wines/{wineId}/tasting-notes",
  operationId: "listWineTastings",
  tags: ["Tastings"],
  summary: "The tastings of one wine, newest first",
  security: [{ FirebaseBearer: [] }],
  request: { params: WineIdPathSchema },
  responses: {
    200: {
      content: { "application/json": { schema: TastingHistoryResponseSchema } },
      description: "Every submitted tasting of the wine the reader may see.",
    },
    401: {
      content: { "application/json": { schema: ErrorEnvelopeSchema } },
...
export function registerTastingSessionRoutes(app: OpenAPIHono<ApiEnvironment>) {
  ...
  app.openapi(wineTastingsRoute, async (context) => {
    const params = context.req.valid("param");
    const history = await listWineTastings(context.env.DB!, {
      principal: context.get("principal"),
    ...
    return history === null
      ...
      : context.json(TastingHistoryResponseSchema.parse({ data: history }), 200);
  });

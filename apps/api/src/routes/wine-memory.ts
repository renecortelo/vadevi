 ...
 * One wine, by its id.
 *
 * The evidence screen used to find a wine by listing a hundred of them and
 * filtering in the browser, which quietly stopped working at wine 101: the page
 * lost its title, its producer and its last venue while the facts below loaded
 * fine. A wine is addressable; fetching it by address costs one row.
 */
const getWineRoute = createRoute({
  method: "get",
  path: "/api/v1/spaces/{spaceId}/wines/{wineId}",
  operationId: "getWine",
  tags: ["Wine memory"],
  summary: "Read one wine by id",
  security: [{ FirebaseBearer: [] }],
  request: { params: WineIdPathSchema },
  responses: {
    200: {
...
const regionPointsRoute = createRoute({
  method: "get",
  path: "/api/v1/spaces/{spaceId}/wines/region-points",
  operationId: "listRegionPoints",
  tags: ["Wine memory"],
  summary: "The reader's wines placed by region, for the map",
  security: [{ FirebaseBearer: [] }],
  request: {
    params: SpaceIdPathSchema,
    query: z.object({ locale: SupportedLocaleSchema.optional() }).strict(),
  },
  responses: {
    200: {
      content: { "application/json": { schema: RegionPointsResponseSchema } },
      description: "One point per region the reader has a wine from, geocoded and cached.",
    },
    401: {
      content: { "application/json": { schema: ErrorEnvelopeSchema } },
    ...
    404: {
      content: { "application/json": { schema: ErrorEnvelopeSchema } },
      description: "The Space is unavailable to the caller.",
    },
  },
});
...
export function registerWineMemoryRoutes(app: OpenAPIHono<ApiEnvironment>) {
  ...
  app.openapi(regionPointsRoute, async (context) => {
    const params = context.req.valid("param");
    const points = await regionPoints(
      context.env.DB!,
      createPlaceSearchPort(context.env.DB!, context.env),
      {
        locale: context.req.valid("query").locale ?? "es",
        principal: context.get("principal"),
    ...
    return points === null
      ...
      : context.json(RegionPointsResponseSchema.parse({ data: points }), 200);
  });

  app.openapi(getWineRoute, async (context) => {
    const params = context.req.valid("param");
    ...
    return wine === null
      ...
      : context.json(WineResponseSchema.parse({ data: { wine } }), 200);
  });

...
describe("Deep tasting and collaborative sessions", () => {
  it("lists a wine's tastings, sharing scores but never a peer's place", async () => {
    const { owner, spaceId } = await sharedSpace();
    const wine = await createWine(spaceId, "Vi d'Historial");

    const now = new Date().toISOString();
    const earlier = new Date(Date.now() - 86_400_000).toISOString();
    // Real note ids are ULIDs, which is what the response contract requires.
    const ownNote = ulid();
    const peerNote = ulid();
    const peer = await bootstrap(memberToken);
    await env.DB.batch([
      env.DB.prepare(
        `INSERT INTO tasting_notes
          (id, space_id, wine_id, author_user_id, mode, state, tasted_at, score_100, version, created_at, updated_at)
          VALUES (?, ?, ?, ?, 'quick', 'submitted', ?, 88, 1, ?, ?)`,
      ).bind(ownNote, spaceId, wine.id, owner.data.user.id, earlier, now, now),
      env.DB.prepare(
      ...
      ).bind(ownNote, spaceId, now, now),
      env.DB.prepare(
        `INSERT INTO tasting_notes
          (id, space_id, wine_id, author_user_id, mode, state, tasted_at, score_100, version, created_at, updated_at)
          VALUES (?, ?, ?, ?, 'quick', 'submitted', ?, 92, 1, ?, ?)`,
      ).bind(peerNote, spaceId, wine.id, peer.data.user.id, now, now, now),
      env.DB.prepare(
        `INSERT INTO tasting_contexts
          (tasting_note_id, space_id, created_at, updated_at, venue_name, venue_latitude, venue_longitude)
          VALUES (?, ?, ?, ?, 'PEER_PRIVATE_BAR', 19.4326, -99.1332)`,
      ).bind(peerNote, spaceId, now, now),
    ]);

    const response = await SELF.fetch(
      `https://vadevi.test/api/v1/spaces/${spaceId}/wines/${wine.id}/tasting-notes`,
    ...
    // Newest first, so the peer's tasting leads.
    expect(body.data).toHaveLength(2);
    expect(body.data[0]?.id).toBe(peerNote);
    // Their score is shared inside a Space; their place is not.
    expect(body.data[0]?.score100).toBe(92);
    expect(body.data[0]?.isSelf).toBe(false);
    expect(body.data[0]?.venueName).toBeNull();
    expect(JSON.stringify(body)).not.toContain("PEER_PRIVATE_BAR");

    // The reader's own row carries its place, and is the one marked openable.
    const mine = body.data.find((entry) => entry.id === ownNote);
    expect(mine?.isSelf).toBe(true);
    expect(mine?.venueName).toBe("Can Pau");
  });

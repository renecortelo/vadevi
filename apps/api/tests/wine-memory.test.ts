...
import { regionPoints } from "../src/repositories/wine-memory";
...
const ownerUid = "firebase-emulator-user-phase-2-owner";
...
describe("Wine Memory and Quick Log", () => {
  it("reads one wine by id, and reaches a wine past the first page", async () => {
    const owner = await bootstrap(ownerToken);
    const spaceId = owner.data.user.activeSpaceId!;

    // Two wines, and a cursor walk between them. Listing with limit 1 is the
    // same shape as listing with limit 100 on a cellar of 101: the question is
    // whether anything past the first page is reachable at all.
    const first = await createWine(spaceId, {
      displayName: "Primera Ficha",
      identityStatus: "confirmed",
      nonVintage: false,
      producerName: "Celler Sintetic",
      vintageYear: 2020,
      wineType: "red",
    });
    const second = await createWine(spaceId, {
      displayName: "Segona Ficha",
      identityStatus: "confirmed",
      nonVintage: false,
      producerName: "Celler Sintetic",
      vintageYear: 2021,
    ...
    const firstId = CreateWineResponseSchema.parse(await first.response.json()).data.wine.id;
    const secondId = CreateWineResponseSchema.parse(await second.response.json()).data.wine.id;

    // By id, without listing anything.
    const direct = await SELF.fetch(
      `https://vadevi.test/api/v1/spaces/${spaceId}/wines/${firstId}`,
      { headers: { Authorization: `Bearer ${ownerToken}` } },
    );
    expect(direct.status).toBe(200);
    const wine = WineResponseSchema.parse(await direct.json()).data.wine;
    expect(wine.id).toBe(firstId);
    expect(wine.displayName).toBe("Primera Ficha");

    // A wine in another Space, or one that does not exist, is not readable.
    const missing = await SELF.fetch(
      `https://vadevi.test/api/v1/spaces/${spaceId}/wines/${ulid()}`,
      { headers: { Authorization: `Bearer ${ownerToken}` } },
    );
    expect(missing.status).toBe(404);

    // Page one of one, then follow the cursor to reach the other wine.
    const pageOne = await SELF.fetch(
      `https://vadevi.test/api/v1/spaces/${spaceId}/wines?limit=1&sort=name`,
      { headers: { Authorization: `Bearer ${ownerToken}` } },
    );
    const one = WineMemoryResponseSchema.parse(await pageOne.json());
    expect(one.data).toHaveLength(1);
    expect(one.page.hasMore).toBe(true);
    expect(one.page.nextCursor).not.toBeNull();

    const pageTwo = await SELF.fetch(
      `https://vadevi.test/api/v1/spaces/${spaceId}/wines?limit=1&sort=name&cursor=${encodeURIComponent(one.page.nextCursor!)}`,
      { headers: { Authorization: `Bearer ${ownerToken}` } },
    );
    const two = WineMemoryResponseSchema.parse(await pageTwo.json());
    // The second page holds a different wine, and between them both are seen.
    expect(two.data[0]?.id).not.toBe(one.data[0]?.id);
    expect([one.data[0]?.id, two.data[0]?.id].sort()).toEqual([firstId, secondId].sort());
  });

  it("carries the reader's own last venue on the wine, and never a peer's", async () => {
    const owner = await bootstrap(ownerToken);
    const spaceId = owner.data.user.activeSpaceId!;
    const created = await createWine(spaceId, {
      displayName: "Vinya del Lloc",
      identityStatus: "confirmed",
      nonVintage: false,
      producerName: "Celler Sintetic",
      vintageYear: 2023,
    ...
    const wineId = CreateWineResponseSchema.parse(await created.response.json()).data.wine.id;

    const now = new Date().toISOString();
    const earlier = new Date(Date.now() - 86_400_000).toISOString();
    const peerId = ulid();
    const ownNoteId = ulid();
    const peerNoteId = ulid();
    await env.DB.batch([
      env.DB.prepare(
        `INSERT INTO users (
      ...
      // The reader's own tasting, at a place with a point.
      env.DB.prepare(
        `INSERT INTO tasting_notes
          (id, space_id, wine_id, author_user_id, mode, state, tasted_at, version, created_at, updated_at)
          VALUES (?, ?, ?, ?, 'quick', 'submitted', ?, 1, ?, ?)`,
      ).bind(ownNoteId, spaceId, wineId, owner.data.user.id, earlier, now, now),
      env.DB.prepare(
        `INSERT INTO tasting_contexts
          (tasting_note_id, space_id, created_at, updated_at, venue_name, venue_latitude, venue_longitude)
          VALUES (?, ?, ?, ?, 'Can Pau', 41.385123, 2.1734)`,
      ).bind(ownNoteId, spaceId, now, now),
      // A co-member drank the same wine somewhere else, more recently.
      env.DB.prepare(
        `INSERT INTO tasting_notes
          (id, space_id, wine_id, author_user_id, mode, state, tasted_at, version, created_at, updated_at)
          VALUES (?, ?, ?, ?, 'quick', 'submitted', ?, 1, ?, ?)`,
      ).bind(peerNoteId, spaceId, wineId, peerId, now, now, now),
      env.DB.prepare(
        `INSERT INTO tasting_contexts
          (tasting_note_id, space_id, created_at, updated_at, venue_name, venue_latitude, venue_longitude)
          VALUES (?, ?, ?, ?, 'ANA_PRIVATE_BAR', 19.4326, -99.1332)`,
      ).bind(peerNoteId, spaceId, now, now),
    ]);

    const listed = await SELF.fetch(`https://vadevi.test/api/v1/spaces/${spaceId}/wines`, {
      headers: { Authorization: `Bearer ${ownerToken}` },
    });
    const body = WineMemoryResponseSchema.parse(await listed.json());
    const wine = body.data.find((entry) => entry.id === wineId);
    expect(wine?.lastVenue?.name).toBe("Can Pau");
    expect(wine?.lastVenue?.latitude).toBe(41.385123);
    // Ana's place is more recent, and must not be the one shown: a venue is
    // location data about a person, and a wine card is not where it belongs.
  ...
  it("places the reader's wines by region, geocoding once and caching", async () => {
    const owner = await bootstrap(ownerToken);
    const spaceId = owner.data.user.activeSpaceId!;
    for (const region of ["Somontano", "Somontano", "Bierzo"]) {
      const created = await createWine(spaceId, {
        displayName: `Vi de ${region} ${randomOpaqueToken().slice(0, 6)}`,
        identityStatus: "confirmed",
        nonVintage: false,
        producerName: "Celler Regio",
        region,
        vintageYear: 2022,
        wineType: "red",
      });
    ...
    // A stand-in geocoder, so the test never reaches Nominatim; it counts calls
    // for the two regions it is asked about, to prove each is resolved once.
    const seen = new Set<string>();
    const place = {
      reverse: async () => ({ cached: false, data: [], status: "success" as const }),
      search: async (input: { query: string }) => {
        if (input.query === "Somontano" || input.query === "Bierzo") seen.add(input.query);
        const point =
          input.query === "Somontano"
            ? { latitude: 42.1, longitude: 0.1 }
            : { latitude: 42.6, longitude: -6.6 };
        return {
          cached: false,
          data: [
            {
    ...
    const first = await regionPoints(env.DB, place, { locale: "es", principal, spaceId });
    expect(first).not.toBeNull();
    // Each of the two regions asked about exactly once — Somontano's two wines
    // did not each trigger a lookup.
    expect(seen.has("Somontano")).toBe(true);
    expect(seen.has("Bierzo")).toBe(true);
    const somontano = first!.find((point) => point.region === "Somontano");
    expect(somontano?.latitude).toBe(42.1);
    expect(somontano?.wines).toHaveLength(2);

    // A second open geocodes nothing new for these regions: the cache answers.
    seen.clear();
    await regionPoints(env.DB, place, { locale: "es", principal, spaceId });
    expect(seen.has("Somontano")).toBe(false);
    expect(seen.has("Bierzo")).toBe(false);
  });

  it("returns no point for a region when no geocoder is configured", async () => {
    const owner = await bootstrap(ownerToken);
    const spaceId = owner.data.user.activeSpaceId!;
    const region = `Zona ${randomOpaqueToken().slice(0, 8)}`;
    const created = await createWine(spaceId, {
      displayName: "Vi sense punt",
      identityStatus: "confirmed",
      nonVintage: false,
      producerName: "Celler Regio",
      region,
    ...
    // Provider off: a region never geocoded simply has no point, rather than
    // failing the map. Other, already-cached regions may still appear, so this
    // asserts on the new region alone.
    const points = await regionPoints(env.DB, null, { locale: "es", principal, spaceId });
    expect(points?.some((point) => point.region === region)).toBe(false);
  });

  it("routes /wines/region-points to the map, not to a wine called region-points", async () => {
    const owner = await bootstrap(ownerToken);
    const spaceId = owner.data.user.activeSpaceId!;
    const response = await SELF.fetch(
      `https://vadevi.test/api/v1/spaces/${spaceId}/wines/region-points`,
      { headers: { Authorization: `Bearer ${ownerToken}` } },
    );
    // The static path wins over /wines/{wineId}; a 200 with a data array proves
    // it reached the region-points handler and not the single-wine 404.
    expect(response.status).toBe(200);

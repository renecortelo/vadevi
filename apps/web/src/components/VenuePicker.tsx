...
export function VenuePicker({
  ...
  // The coordinates box is its own field now: the name box only ever names, so a
  // reader both names the place and locates it, rather than one field guessing
  // which they meant.
  ...
  return (
    <div className="venue-picker">
      ...
      {/* A separate box for coordinates copied from a map. It fills the point and
          leaves the name alone, so a place OpenStreetMap does not know is still
          both located and named. */}
      <label className="venue-picker__coords">
        <span>{t("tasting.venue.pasteLabel")}</span>
        <input
          inputMode="text"
          onChange={(event) => {
            ...
            const pasted = parsePastedPoint(text);
            if (pasted === null) {
              setNotice(t("tasting.venue.pasteUnrecognized"));
              return;
            }
            onChoose({
              area: null,
              ...
              // The name stays whatever the reader typed above; only the point
              // comes from here.
              name: value.trim(),
            });
            setCoords("");
          ...
          placeholder={t("tasting.venue.pastePlaceholder")}
          value={coords}
        />
      </label>

      {/* Once a point is captured, say so, so it is clear the place is located
          even before a name is typed. */}
      {latitude === null ||
      latitude === undefined ||
      longitude === null ||
      longitude === undefined ? null : (
        <p className="venue-picker__located">
          {t("tasting.venue.located", {
      ...
      <p className="venue-picker__hint">{t("tasting.venue.hint")}</p>

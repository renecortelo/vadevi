...
export function QuickLogPage() {
  ...
  /** Refresh the prefill candidates from the server as the reader types, so a
   *  wine past the first page can be reached at all. */
  async function searchWines(query: string) {
    ...
    try {
      ...
      // Keep whatever is already offered; prefilling is a convenience.
    }
  }

  const prefillSearch = useWineSearch(wines, searchWines);
          ...
          {wines.length === 0 ? null : (
            <label className="quick-log-prefill" htmlFor="prefill-from">
              ...
              <input
                ...
                onChange={(event) => prefillSearch.setFilter(event.target.value)}
                placeholder={t("winePicker.filterPlaceholder")}
                type="search"
                value={prefillSearch.filter}
              />
              <select
                id="prefill-from"
                onChange={(event) => {
              ...
              >
                ...
                {prefillSearch.shown.map((candidate) => (

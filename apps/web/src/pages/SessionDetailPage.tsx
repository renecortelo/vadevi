...
export function SessionDetailPage() {
  ...
  async function addSelected() {
    ...
    try {
      await queueSessionWines({
        ...
        spaceId,
        userId: user.uid,
        // From the choice itself: searching replaces what is visible, and a
        // wine ticked before a search would otherwise be dropped from the flight.
        wines: choice.chosen,
      });
      choice.clear();
      await refreshStatus();
  ...
  async function searchWines(query: string) {
    ...
    try {
      ...
      setAvailableWines(response.data);
    } catch {
      // Keep whatever is already offered.
    }
  }

  const addSearch = useWineSearch(winesNotInFlight, searchWines);
  // A wine already ticked stays on screen even when the search stops returning
  // it, so it can be unticked rather than becoming invisible and still added.
  const addVisible = [
    ...addSearch.shown,
    ...choice.chosen.filter((wine) => !addSearch.shown.some((shown) => shown.id === wine.id)),
  ] as WineSummary[];
        ...
        {winesNotInFlight.length === 0 ? null : (
          <>
            <input
              ...
              onChange={(event) => addSearch.setFilter(event.target.value)}
              placeholder={t("winePicker.filterPlaceholder")}
              type="search"
              value={addSearch.filter}
            />

import type { RegionPointsResponse, TastingSessionResponse, WineSummary } from "@vadevi/contracts";
...
export function WineMemoryPage() {
  ...
  const [mapKind, setMapKind] = useState<"region" | "venue">("venue");
  const [regionPoints, setRegionPoints] = useState<RegionPointsResponse["data"]>([]);
  ...
  const [view, setView] = useState<"cards" | "map" | "sessions" | "table" | "timeline">("cards");
  ...
  useEffect(() => {
    if (user === null || view !== "map" || mapKind !== "region") return;
    const controller = new AbortController();
    void getRegionPoints(user, spaceId, i18n.language, controller.signal)
      .then(setRegionPoints)
      .catch(() => setRegionPoints([]));
    return () => controller.abort();
  }, [i18n.language, mapKind, spaceId, user, view]);

  /** The next page, appended. Filters and sort are already in the state the
   *  first page was fetched with, so only the cursor moves. */
  async function loadMore() {
    if (user === null || nextCursor === null || loadingMore) return;
    setLoadingMore(true);
    try {
      ...
      // Appended rather than replaced, and de-duplicated by id: a wine added
      // between the two reads can otherwise arrive on both pages.
      setWines((current) => {
        const seen = new Set(current.map((wine) => wine.id));
        return [...current, ...response.data.filter((wine) => !seen.has(wine.id))];
      });
  ...
  return (
    <section className="memory-page">
      ...
      */}
      ...
      {view !== "sessions" && view !== "map" && wines.length === 0 ? (
      ...
      {/* How many are shown, and a way to the rest. The list asks the server for
          a page at a time; without this the cellar simply stopped at whatever
          the first page held, with nothing on screen to say more existed. */}
      {view !== "sessions" && view !== "map" && wines.length > 0 ? (
        <div className="memory-paging">
          <p aria-live="polite">{t("memory.shownCount", { count: wines.length })}</p>
      ...
      {view === "map" ? (
        <div className="memory-map">
          <div aria-label={t("memory.mapKindLabel")} className="segmented-control" role="group">
            <button
              aria-pressed={mapKind === "venue"}
              onClick={() => setMapKind("venue")}
              type="button"
            >
              {t("memory.mapVenue")}
            </button>
            <button
              aria-pressed={mapKind === "region"}
              onClick={() => setMapKind("region")}
              type="button"
            >
              {t("memory.mapRegion")}
            </button>
          ...
          {(() => {
            const venuePoints: MapPoint[] = wines.flatMap((wine) =>
              wine.lastVenue === undefined ||
              wine.lastVenue.latitude === null ||
              wine.lastVenue.longitude === null
                ? []
                : [
                    {
                      latitude: wine.lastVenue.latitude,
                      longitude: wine.lastVenue.longitude,
                      subtitle: wine.lastVenue.name,
                      title: `${wine.producerName} · ${wine.displayName}`,
                      wineId: wine.id,
                    },
                  ],
            );
            const regions: MapPoint[] = regionPoints.flatMap((point) =>
              point.wines.map((wine) => ({
                latitude: point.latitude,
                longitude: point.longitude,
                subtitle: point.region,
                title: wine.name,
                wineId: wine.id,
              })),
            );
            const active = mapKind === "venue" ? venuePoints : regions;
            const empty = mapKind === "venue" ? t("memory.mapEmpty") : t("memory.mapRegionEmpty");
            // A real map whenever the deployment serves tiles — empty or not, so
            // it draws the world even before any point exists rather than hiding
            // behind the schematic. The schematic is only the no-tiles fallback.
            if (bootstrap.data.features.mapTiles) {
              return (
                <>
                  {active.length === 0 ? <p className="section-help">{empty}</p> : null}
                  <Suspense fallback={<p className="section-help">{t("memory.mapEmpty")}</p>}>
                    <TileMap points={active} />
                  </Suspense>
                </>
              );
            }
            // Tiles are off for this deployment. Say so plainly instead of
            // silently showing the schematic, so a real map that was expected
            // and did not appear is a message, not a mystery.
            return (
              <>
                <p className="section-help">{t("memory.mapSchematicNotice")}</p>
                <PointsMap emptyLabel={empty} points={active} />
              </>
            );
          })()}
        </div>

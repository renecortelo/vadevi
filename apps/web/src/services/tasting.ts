...
/** A wine's tastings, newest first. */
export async function getWineTastings(
  tokenSource: TokenSource,
  spaceId: string,
  wineId: string,
  signal?: AbortSignal,
): Promise<TastingHistoryEntry[]> {
  const response = await authenticatedFetch(
    tokenSource,
    `/api/v1/spaces/${spaceId}/wines/${wineId}/tasting-notes`,
    signal === undefined ? {} : { signal },
  );
  if (!response.ok) throw await apiError(response);
  return TastingHistoryResponseSchema.parse(await response.json()).data;
}

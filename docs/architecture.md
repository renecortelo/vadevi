...
Open Food Facts, Wikidata, Wikipedia, the optional SommelierX pairing provider, and the optional web-search provider (Brave or Tavily) are exposed only through fixed official HTTPS hosts; neither user input nor model output can select a URL. Web search is a discovery provider, not a crawler: it calls one official search API and cites the snippets it returns, and the app never fetches the arbitrary result pages, so the general-web SSRF boundary stays closed. Wikidata is asked only about the wine's grapes: producer and region name lookups attached the wrong entity or the right entity's wrong detail too often, and the open web answers those better. Wikipedia is reached only via the article title stored in a matched Wikidata entity's sitelinks — the same identity footprint as the Wikidata lookup — to add a short, cited summary paragraph. Authorized research jobs persist normalized output as cited proposed facts and never human-verify it. When Workers AI is enabled, gathered web snippets are translated into the reader's language and the research summary is composed by the model from the gathered material — an encyclopedia summary when one exists, otherwise the web notes and highlights — cited to the source it was written from. A separate authorized command rewrites that paragraph from the facts that are still live, so discarding a claim does not require researching again. Dish ideas for one of the reader's own wines are produced the same way, from that wine's own recorded and researched attributes, and are surfaced as inferred suggestions. All of these are faithful transforms that fall back to what they had on failure and add no claim the cited material does not contain. Region country and classification are resolved from an offline curated eAmbrosia gazetteer, and country, appellation, and grape terms from further offline gazetteers, so those facts need no network call at all. An optional Workers AI adapter can render bounded structured statements, but claim-to-statement and sentence-to-source enforcement rejects invented or uncited output. A future non-fixed fetch boundary must add DNS resolution and private-address rechecks before it is allowed.

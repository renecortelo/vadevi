...
## Region points on the map

The map's "where they're from" view needs a point for each wine region, which is
stored only as text. When the place provider is on, a region name is geocoded —
sent to Nominatim exactly as a venue search is — and the result cached in a
shared `region_points` table keyed by the region name. A region's location is
public and identical for everyone, so this cache is not per-Space: geocoding
"Priorat" once places every Priorat. The wine ids shown at a point are the
caller's own; the point is public knowledge, the cellar behind it is not. With
the provider off, no region is geocoded and the region map shows only whatever
was already cached.
...
The map's "real map" background, when `MAP_TILES_PROVIDER=openstreetmap`, is
drawn from OpenStreetMap tiles — but the browser never contacts a tile host. The
Worker fetches each tile and serves it from this origin (`/api/v1/map-tiles/...`),
exactly as bottle photos are proxied, so the strict `default-src 'self'` policy
is unchanged and the reader's address never reaches OSM. A tile is public
geography: it carries no wine, note, or account, and which tiles are fetched

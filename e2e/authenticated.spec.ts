...
test.describe("authenticated screens", () => {
  ...
  const screens = [
    ...
    // Everything built since this list was written. The two most complex screens
    // in the application — the deep tasting form and the evidence page — had
    // never been checked at all.
    { path: "/sessions", name: "events" },
    { path: "/sessions/new", name: "new event" },
    { path: "/wishlist", name: "wishlist" },
    { path: "/shop", name: "shop" },
    { path: "/vicenc", name: "assistant" },
  ...
  test("has no serious or critical axe violation on a wine's own screens", async ({ page }) => {
    // The deep tasting form and the evidence page are the largest screens here
    // and had never been checked: they need a wine to exist first, which is the
    // only reason they were left out of the list above.
    await page.goto("/log/new");
    await page.waitForLoadState("networkidle");
    await page
      .getByLabel(/producer/i)
      .first()
      .fill("Celler Accessible");
    await page
      .getByLabel(/wine name/i)
      .first()
      .fill("Vinya Accessible");
    ...
    for (const path of [`/wines/${wineId}/evidence`, `/wines/${wineId}/taste`]) {
...
test.describe("authenticated screens at 320 CSS px", () => {
  ...
  test("has no horizontal overflow on the main flow", async ({ page }) => {
    ...
    // The phone is the device this application is actually used on, and a
    // horizontal overflow there is a finding rather than a nitpick — the bottle
    // photo grid spilled past the margin once and only a person noticed.
    for (const path of [
      "/",
      "/memory",
      "/log/new",
  ...
  test("has no horizontal overflow on a wine's own screens", async ({ page }) => {
    await signIn(page);
    await completeOnboarding(page);

    // Evidence and the tasting form need a wine to be about. They are the two
    // most complex screens in the application and the ones a phone spends the
    // most time on; the bottle-photo grid overflowed on evidence once.
    await page.goto("/log/new");
    await page.waitForLoadState("networkidle");
    await page
      .getByLabel(/producer/i)

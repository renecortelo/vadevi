...
export const DeepTastingFieldsSchema = z
  .object({
    ...
    beadSize: z.enum(["fine", "medium", "coarse"]).optional(),
    ...
    effervescence: TastingScaleSchema.optional(),
    ...
    noseSwirledIntensity: TastingScaleSchema.optional(),
    noseSwirledText: z.string().trim().max(2_000).optional(),
...
/**
 * One past tasting of a wine, as a history line.
 *
 * What a co-member contributes is what is already shared within a Space: who
 * they are, when they drank it and how they scored it. Their written notes stay
 * with them — and so does their venue, which is location data about a person —
 * so only the reader's own rows carry those, and only the reader's own rows can
 * be opened in full.
 */
export const TastingHistoryEntrySchema = z
  .object({
...
export const TastingHistoryResponseSchema = z
  .object({ data: z.array(TastingHistoryEntrySchema).max(50) })
  .strict()
  .openapi("TastingHistoryResponse");
...
export type TastingHistoryEntry = z.infer<typeof TastingHistoryEntrySchema>;
export type TastingHistoryResponse = z.infer<typeof TastingHistoryResponseSchema>;

...
/**
 * The place of the reader's own most recent tasting of a wine.
 *
 * Deliberately their OWN: a venue is location data about a person, and a wine
 * card is a far more prominent place than the tasting it came from. A
 * co-member's place stays on their tasting, where it already was.
 */
export const LastVenueSchema = z
  .object({
...
/**
 * A wine region placed on the map, with the wines from it.
 *
 * The point is the region's, shared across every cellar; the wine ids are the
 * caller's own, so a click on "Priorat" opens the reader's Priorats and no one
 * else's.
 */
export const RegionPointSchema = z
  .object({
    ...
    wines: z
      .array(z.object({ id: ResourceIdSchema, name: z.string().min(1).max(400) }).strict())
      .max(500),
  })
  .strict();

export const RegionPointsResponseSchema = z
  .object({ data: z.array(RegionPointSchema).max(500) })
  .strict()
  .openapi("RegionPointsResponse");
...
export const WineSummarySchema = z
  .object({
    ...
    // Where the reader last drank this bottle, so a wine can be traced back to
    // the evening rather than only to a date. Optional rather than nullable on
    // purpose: offline snapshots cached before this existed must keep parsing.
    lastVenue: LastVenueSchema.optional(),
...
export type RegionPointsResponse = z.infer<typeof RegionPointsResponseSchema>;
